...
export const PlayerCard: React.FC<PlayerCardProps> = ({
    ...
    return (
      <div
        ...
        className={`flex items-center justify-between p-2 rounded-xl border transition ${
          isFavorite
            ? 'bg-blue-50/90 border-[#1b55e2] ring-1 ring-[#1b55e2] dark:bg-blue-950/40 dark:border-blue-700'
      ...
      >
        ...
        <button
          ...
          className={`p-1.5 rounded-lg text-xs font-black transition ${
            isFavorite
              ? 'bg-rose-50 text-rose-600 dark:bg-rose-950/40 hover:bg-rose-600 hover:text-white'
  ...
  return (
    <div
      ...
      className={`group relative flex flex-col justify-between p-3.5 rounded-xl border transition hover:shadow-sm ${
        isFavorite
          ? 'bg-blue-50/70 border-[#1b55e2] dark:bg-blue-950/30 dark:border-blue-700 ring-1 ring-[#1b55e2]'
          : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-300 shadow-xs'
      }`}
    >
        ...
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <h3 className="font-black text-slate-900 dark:text-slate-100 text-xs sm:text-sm leading-snug truncate group-hover:text-[#1b55e2] transition-colors">
                {player.nombre}
              </h3>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between pt-2.5 border-t border-slate-100 dark:border-slate-800 mt-2">
        <div>
          <span className="text-[9px] uppercase font-black text-slate-400 dark:text-slate-500 block">
            Cotización
          </span>
          <span className="text-sm font-black font-mono tracking-tight text-emerald-700 dark:text-emerald-400">
            {formattedPrice}
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-lg text-xs font-black transition active:scale-95 flex items-center gap-1 shadow-xs ${
              isFavorite
                ? 'bg-blue-100 text-[#1b55e2] dark:bg-blue-900/60 dark:text-cyan-300 hover:bg-rose-500 hover:text-white border border-blue-200 dark:border-blue-700'
